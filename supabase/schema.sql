...
create policy "quotes_sindico" on quotes for all using (sindico_id = auth.uid() or is_admin())
  with check (sindico_id = auth.uid());
create policy "quotes_empresa_read" on quotes for select using (
  id in (select quote_id from quote_companies where company_id = my_company_id())
);

create policy "quote_photos_access" on quote_photos for all using (
  quote_id in (select id from quotes where sindico_id = auth.uid())
  or quote_id in (select quote_id from quote_companies where company_id = my_company_id())
  or is_admin()
) with check (quote_id in (select id from quotes where sindico_id = auth.uid()));

create policy "quote_companies_sindico" on quote_companies for all using (
  quote_id in (select id from quotes where sindico_id = auth.uid()) or is_admin()
) with check (quote_id in (select id from quotes where sindico_id = auth.uid()));
...
create policy "quote_slots_access" on quote_slots for all using (
  quote_id in (select id from quotes where sindico_id = auth.uid())
  or quote_id in (select quote_id from quote_companies where company_id = my_company_id())
  or is_admin()
) with check (quote_id in (select id from quotes where sindico_id = auth.uid()));
...
create policy "appointments_access" on appointments for all using (
  quote_company_id in (
    select qc.id from quote_companies qc
    join quotes q on q.id = qc.quote_id
    where q.sindico_id = auth.uid() or qc.company_id = my_company_id()
  ) or is_admin()
) with check (
  quote_company_id in (select id from quote_companies where company_id = my_company_id())
);

create policy "proposals_access" on proposals for all using (
  quote_company_id in (
    select qc.id from quote_companies qc
    join quotes q on q.id = qc.quote_id
    where q.sindico_id = auth.uid() or qc.company_id = my_company_id()
